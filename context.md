...
### Implemented
- **v3 "Heritage Gold & Forest" design (Stitch migration, 2026-08-11)**: deep forest green (`#1B3428` canvas, `#00180e` deepest) + antique gold (`#C9A24B`) + warm ivory plaques; Bodoni Moda + Manrope; full-page restyle of nav, hero (Jharokha frame + brand headline), step-well divider, category grid, footer, modal, and cart drawer.
- **Fixed heritage backdrop**: one full-screen gold Rani ki Vav **monument line-art** layer (inline sprite in `frontend/index.html`) behind every section at low rest opacity with an edge vignette.
...
- **Motif sprite**: the storefront inlines a hidden sprite in `frontend/index.html` (symbols `stepwell-motif`, `stepwell-motif-slim`, `stepwell-frieze`, `stepwell-shaft`). Every motif layer is `aria-hidden` + `pointer-events: none` (decorative only).
...
| Motif artwork | Inline sprite in `frontend/index.html` | The full stepwell composition lives inline in the page. |
